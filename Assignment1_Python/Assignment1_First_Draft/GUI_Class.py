...
class MyWindow():
    ...
    def AddImage(self, file_name, struct):
        ...
        self.OneImage.grid(row = struct['row'], rowspan = struct['rowspan'], column = struct['column'] , \
            columnspan = struct['columnspan'], sticky = struct['sticky'], padx = struct['padx'], pady = struct['pady'])  
    ...
    def AddStatus(self, para, struct,  text_add ):
        self.OneStatus = Label(self.root, text = text_add, bd = para['bd'], fg = para['fg'], relief = SUNKEN, anchor = para['anchor'])
        self.OneStatus.grid(row = struct['row'], columnspan = struct['columnspan'], sticky = struct['sticky'])

    def AddText(self,struct,text_add):
        self.OneText = Text(self.root)
        self.OneText.insert(INSERT, text_add)
        self.OneText.grid(row = struct['row'], column = struct['column'], columnspan = struct['columnspan'], sticky = struct['sticky'])
