...
def LoginButtonFunc(): 
    ...
    try:
        ...
        if answer == 'yes': 
            ...
            var_TabCheck.set("Tables checking, if not exist, creat new table")
            Stat_Para_TabCheck_exist = {'bd':1,'anchor':W, 'fg':'black'}
            win.AddStatus(Stat_Para_TabCheck_exist, TabCheckStructStatus, var_TabCheck.get())
            win.root.update()

        
        var_TabCheck.set('Table checking passed!\nPlease input the following file names')
...
def ConfirmButtonFunc():
    ...
    if identify == 3: 
        var_TotStatus.set('Failed! Please check above message!')

        Stat_Para_Tot_fail= {'bd':1,'anchor':W, 'fg':'red'}
        win.AddStatus(Stat_Para_Tot_fail, TotStructStatus, var_TotStatus.get())
        win.root.update()
        ...
        try:
            cur_test = cur
            conn_test = conn
            var_WriteStatus.set('Writing Tables...')
            ...
            var_TotStatus.set('''Succeed! Please Exit and check Database 5bus!\nServer Now Disconnected for Protection!''')

            Stat_Para_Tot_success= {'bd':1,'anchor':W, 'fg':'#228B22'}
            win.AddStatus(Stat_Para_Tot_success, TotStructStatus, var_TotStatus.get())
            win.root.update()
            tkMessageBox.showinfo(title='Computer Application', message='All Tables Are Now Ready!')
        except:
            tkMessageBox.showinfo(title='Computer Application', message='No Connection to MySQL Server! Please First Login!')
            var_WriteStatus.set('No Connection to MySQL Server! Please First Login!')
...
FileNameStructLab = {'row':10,'columnspan':4, 'sticky':N+E+S+W, 'rowspan':1, 'column':0}
win.AddLabel('Input the file name here' , Lab_Para_FileName, FileNameStructLab)



Lab_Para_EQName = {'bg':None, 'fg':'black'}
EQNameStructLab = {'row':11,'columnspan':1, 'sticky':E, 'rowspan':1, 'column':0}
...
EQNameStructEntr = {'row':11, 'column':1,'sticky':W}
win.AddEntry(var_EQName, Entr_Para_EQName , EQNameStructEntr)


Lab_Para_SSHName = {'bg':None, 'fg':'black'}
SSHNameStructLab = {'row':12,'columnspan':1, 'sticky':E, 'rowspan':1, 'column':0}
...
SSHNameStructEntr = {'row':12, 'column':1,'sticky':W}
win.AddEntry(var_SSHName, Entr_Para_SSHName , SSHNameStructEntr )



ConfirmStructButt = {'row':13,'column':2,'padx':4,'pady':1}
win.AddButton('Confirm', ConfirmButtonFunc, ConfirmStructButt)

Exit2StructButt = {'row':13,'column':3,'padx':4,'pady':1}
...
EQCheckStructStatus = {'row':14, 'sticky':N+E+S+W, 'columnspan':3}
...
SSHCheckStructStatus = {'row':15, 'sticky':N+E+S+W, 'columnspan':3}
